...
public interface BuildingViewRepository extends JpaRepository<Building, String> {
    ...
    @Query("SELECT b.name AS name, b.id AS id, b.location.id AS locationId FROM Building b")
    List<BuildingView> findAllBuildingViews();


    @Query("SELECT b.name AS name, b.id AS id, b.location.id AS locationId FROM Building b WHERE b.id= :id")
    BuildingView findBuildingViewById(@Param("id") String id);

    @Query("SELECT b.id as id, b.name as name, b.location.id as locationId FROM Building b WHERE b.location.id= :locationId")
