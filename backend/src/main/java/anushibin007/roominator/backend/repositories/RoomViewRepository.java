...
public interface RoomViewRepository extends JpaRepository<Room, String> {
    @Query("SELECT r.id as id, r.name as name, r.building.id as buildingId, r.floor as floor, r.capacity as capacity  FROM Room r ORDER BY r.name")
...
//    RoomView findRoomViewById(@Param("id") String id);
    ...
    @Query("SELECT r.id as id, r.name as name, r.building.id as buildingId, r.floor as floor, r.capacity as capacity FROM Room r WHERE r.building.id= :building ORDER BY r.name")
